...
- (void)viewDidLoad
{
    ...
    levelLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.frame.size.width/2 - 260/2, 210, 260, 40)];
    levelLabel.textColor=[UIColor whiteColor];
    levelLabel.textAlignment=NSTextAlignmentCenter;
    levelLabel.font=[UIFont fontWithName:Nil size:20];
...
-(void)highliteNewButton{
    ...
    buttonTagToPress= arc4random() %35;

    while (buttonTagToPress==0) {
        buttonTagToPress= arc4random() %35;
    }
...
-(void)endGame{
    ...
    if (score>10) {
        levelLabel.text= [NSString stringWithFormat:@"Level %i - Successfull!", level];
        level++;
    ...
    int width = self.view.frame.size.width/2 - 150/2;
    scoreLabel1 = [[UILabel alloc]initWithFrame:CGRectMake(width, 250, 150, 40)];
    scoreLabel1.textColor=[UIColor whiteColor];
    scoreLabel1.textAlignment=NSTextAlignmentCenter;
    scoreLabel2.font=[UIFont fontWithName:Nil size:20];

    scoreLabel1.text= [NSString stringWithFormat:@"Your Score is"];
    [self.view addSubview:scoreLabel1];

    scoreLabel2 = [[UILabel alloc]initWithFrame:CGRectMake(width, 300, 150, 40)];
    scoreLabel2.textColor=[UIColor whiteColor];
    scoreLabel2.textAlignment=NSTextAlignmentCenter;
    scoreLabel2.font=[UIFont fontWithName:Nil size:40];
